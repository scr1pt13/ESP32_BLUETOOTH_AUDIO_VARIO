...
static void vario_task(void *pvParameter)
{
	...
	Imu.config_accel_gyro();   // configure MPU9250 to start generating gyro, accel data	
	ui_calibrate_accel_gyro(); // calibrate gyro (accel  if required)
	...
	while (1)
	{
		...
		if (baroCounter >= 5)
		{					 // 5*2mS = 10mS elapsed, this is the sampling period for MS5611,
			...
			if (zMeasurementAvailable)
			{
				...
				else if (pwrOffTimeoutSecs >= (Config.misc.pwrOffTimeoutMinutes * 60))
				{
